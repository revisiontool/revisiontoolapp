...
function LoginPopup(){
    ...
    return(
        <div className='flex flex-col justify-center items-center px-5 min-h-full' style={{backgroundImage: `url(${background.src})`, backgroundPosition: '50% 25%', backgroundRepeat: 'no-repeat', backgroundSize: 'cover'}}>
            <div className={`${style.headingfont} xl:text-7xl lg:text-5xl md:text-4xl sm:text-3xl text-2xl text-center bg-white`}>
                <p>Welcome Back!</p>
            </div>
            <div className='flex flex-col border-2 border-black rounded-xl sm:px-6 px-3 py-2 mt-5 max-w-xl w-full items-start bg-white z-20'>
                <div className='mt-3'>
                    <p className={`${style.trialfont} xl:text-2xl lg:text-xl md:text-lg sm:text-base text-sm`}>Enter your email and password to access your account and question sets.</p>
                </div>
            ...
            <Image className='absolute vsm:w-28 w-14 bottom-1/20 right-1/10 z-10 sm:hidden' src={chatbox}/>
            <Image className='absolute vsm:w-28 w-14 vsm:top-1/10 top-1/20 left-1/10 z-10 sm:hidden' src={rise}/>
        </div>
    )
}
